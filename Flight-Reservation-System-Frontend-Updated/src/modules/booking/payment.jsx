...
function Payment() {
    ...
    useEffect(() => {
        if (timer > 0) {
            ...
            handleCancelBooking(); // Cancel booking when timer reaches zero
        }
    }, [timer]);

    const handleCancelBooking = async () => {
        try {
            await SeatBookingServices.createReservation(
                userInfoSession.selectedFlightNumber,
                userInfoSession.userId,
                userInfoSession.selectedSeat,
                null, // Price is null for cancellation
                true  // Pass true to indicate payment cancellation
            );
            alert('Payment timeout! Your reservation has been canceled.');
            history.push('/'); // Redirect the user to the home or booking page
        } catch (error) {
