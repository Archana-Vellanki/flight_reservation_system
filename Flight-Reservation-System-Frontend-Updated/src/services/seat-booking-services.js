...
export default class SeatBookingServices {
    ...
    // static bookSeats = (flightNumber, userId, seatNumber, seatClass, price, paymentCancellation = false) => {
    //     const bookSeatsURL = LOCAL_BACKEND_BASE_URL + 'booking/bookSeats/' + flightNumber + '/' + userId;
    //     const bookSeatsRequest = {
    //         seatNumber: seatNumber,
    //         seatClass: seatClass,
    //         price: paymentCancellation ? null : price, // Set price to null if canceling
    //         booked: !paymentCancellation // If canceling, set `booked` to false
    //     };
    ...
    static createReservation = (flightNumber, userId, seatNumber, price, isPaymentCancelled = false) => {
        const createReservationURL = LOCAL_BACKEND_BASE_URL + 'payment/createReservation/' + flightNumber + '/' + userId + '/' + seatNumber;
    
        // Prepare the request payload
        const createReservation = {
            totalTravelFair: isPaymentCancelled ? -1 : price, // Set price to null if canceling the reservation
            // reservationStatus: isPaymentCancelled ? 'CANCELED' : 'CONFIRMED', // Optionally, include a status field
        };
